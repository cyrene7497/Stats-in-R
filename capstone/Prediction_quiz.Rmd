...
# Get dataset of predictions and confidence intervals
out1 = as.data.frame(cbind(exp(confint(pred.test.MPM)),
                          price = ames_test$price))

# Fix names in dataset
colnames(out1)[1:2] <- c("lwr", "upr")  #fix names

# Get Coverage
pred.test.MPM.coverage <- out1 %>% summarize(cover = sum(price >= lwr & price <= upr)/n())
