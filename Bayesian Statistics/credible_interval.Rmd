...
```{r q7}
qbeta(c(0.49, 0.51), shape1 = 5, shape2 = 200)
```
...
a <- 5 + sum(brfss$fruit_per_day)
b <- 1 + length(brfss$fruit_per_day)
a
b
```
...
veg_a <- 5 + sum(brfss$vege_per_day)
veg_b <- 1 + length(brfss$vege_per_day)
veg_a
veg_b
qgamma(c(0.05, 0.95), shape = veg_a, rate = veg_b)
```
